...
const contractAbi = require('./abi/contractAbi04.json');
...
export class EthcontractService {
  ...
  // new version return json
  public async getOffersToSell(): Promise<any[]> {
    return new Promise((resolve, reject) => {
      this._tokenContract.OfferToSell( { address: this._tokenContractAddress }, { fromBlock: 0, toBlock: 'latest' } )
      .get((err, logs) => {
        const accounts = {};
        ...
        const allOffersToSell:any[] = new Array();
        let loops = 0;
        let index = 0;
  ...
  // new version return json
  public async getOffersToBuy(): Promise<any[]> {
    return new Promise((resolve, reject) => {
      this._tokenContract.OfferToBuy( { address: this._tokenContractAddress }, { fromBlock: 0, toBlock: 'latest' } )
      .get((err, logs) => {
        const accounts = {};
        for (let i = 0; i < logs.length; i++) {
          const log = logs[i];
        ...
        const arrayAccounts = Object.keys(accounts);
        const allOffersToBuy:any[] = new Array();
        let loops = 0;
        let index = 0;
        for (let i = 0; i < arrayAccounts.length; i++) {
          this._tokenContract.showOffersToBuy(arrayAccounts[i], (err, result) => {
            ...
            if (result[0]) {
              allOffersToBuy[index] = {
                buyer: arrayAccounts[i], 
                valueLot: result[1], 
                price: result[2], 
                status: result[0] 
              };
